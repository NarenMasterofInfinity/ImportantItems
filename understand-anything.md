...
pnpm understand init
pnpm understand index /path/to/some/repo --no-llm
pnpm understand serve /path/to/some/repo
...
5. Large repo behavior:
- does not render all nodes at once,
- graph API supports neighborhood queries,
- SQLite is used for storage.

6. LLM disabled mode works fully for graph, search, layers, tours, and diff.
7. LLM enabled mode adds summaries, persona explanations, and polished tours.
